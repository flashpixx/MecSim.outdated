    ...
    */
    ...
    private void checkFile( final Path p_file ) throws IOException
    {
        ...
        for( String l_line : Files.readAllLines( p_file ) )
        {
            ...
            //System.out.println("----> " + l_package + "    " + l_classname + "     " + l_parameter[0] + "      " + l_parameter[1]);

            /*
            l_label = l_cmd[1];

            if (!l_cmd[0].equals( "this" ))
                l_classname = l_cmd[0];
            if (!l_classname.contains( "." ))
                l_classname = l_package + "." + l_classname;
            */

            // check label and add label to the set
            //this.checkLabel( l_classname, l_label );
            //m_labels.add( CCommon.removePackageName( l_classname ) + "." + l_label );
        }
    }
    ...
    {
        for( String l_language : CConfiguration.getInstance().get().<ArrayList<String>>getTraverse( "language/allow" ))
        {
            CConfiguration.getInstance().get().setTraverse( "language/current", l_language );
            try
            {
                final String l_label = CCommon.getResourceString( Class.forName( p_classname ), p_label );
                if ( ( l_label == null ) || ( l_label.isEmpty() ) )
                    throw new IllegalStateException();
            }
            catch ( final Exception l_exception )
            {
                fail( String.format( "label [%s] in language [%s] within class [%s] not found", p_label, l_language, p_classname ) );
            }
        }
    }
